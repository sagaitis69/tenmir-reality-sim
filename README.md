...
- **LLM**: Any OpenAI-compatible endpoint; defaults target **Google’s Gemma 4** model id via the Gemini developer API base URL.
...
Edit `.env`: set `OPENAI_API_KEY` from [Google AI Studio](https://aistudio.google.com/apikey) and confirm `LLM_MODEL` matches a model id your key can call (see Google’s Gemma 4 / API docs; examples include `gemma-4-31b-it`).
...
Open **http://127.0.0.1:8787/** for the **glassmorphic dashboard** (landing). The Gemma console lives at **http://127.0.0.1:8787/console** — use **Re-index books folder** there after adding files.

### Local Gemma 4 (optional)

Point `OPENAI_BASE_URL` at **vLLM** or **Ollama** if you serve an OpenAI-compatible Gemma 4 stack locally, and set `LLM_MODEL` to the served model name.

## API

| Method | Path | Purpose |
|--------|------|---------|
| GET | `/api/health` | Corpus chunk count + config sanity |
